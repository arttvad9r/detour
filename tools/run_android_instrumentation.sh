...
API_LEVEL="${1:?usage: run_android_instrumentation.sh <api-level> <emulator-port> [memory-mb] [system-image] [expected-page-size]}"
EMULATOR_PORT="${2:?usage: run_android_instrumentation.sh <api-level> <emulator-port> [memory-mb] [system-image] [expected-page-size]}"
MEMORY_MB="${3:-2048}"
SYSTEM_IMAGE="${4:-system-images;android-${API_LEVEL};google_apis;x86_64}"
...
if [[ -n "$EXPECTED_PAGE_SIZE" ]]; then
  ...
  echo "Android API $API_LEVEL page size: $ACTUAL_PAGE_SIZE"
