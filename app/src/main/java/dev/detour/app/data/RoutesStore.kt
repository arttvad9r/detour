...
class RoutesStore(context: Context) {
    ...
    private val settingsFile = context.preferencesDataStoreFile("detour_settings")

    init {
        val legacyFile = context.preferencesDataStoreFile("trip" + "let_settings")
        if (!settingsFile.exists() && legacyFile.exists()) {
            legacyFile.copyTo(settingsFile)
        }
    }
