...
enum class AppTheme(
    ...
    fun statusFor(state: dev.detour.app.vpn.VpnState): StatusStyle = statusStyleFor(colors, state)
...
fun statusStyleFor(colors: DetourColors, state: dev.detour.app.vpn.VpnState): StatusStyle = when (state) {
    dev.detour.app.vpn.VpnState.Active -> StatusStyle(colors.activeSoft, colors.activeStrong, colors.activeBorder)
    dev.detour.app.vpn.VpnState.Starting -> StatusStyle(colors.accentSoft, colors.accent, colors.accentBorder)
    is dev.detour.app.vpn.VpnState.Failed -> StatusStyle(colors.surface, colors.error, colors.error.copy(alpha = .45f))
    dev.detour.app.vpn.VpnState.Idle -> StatusStyle(colors.surface, colors.textPrimary, colors.border)
}
